...
function SAEO_Blitter() {
    ...
    this.SAEF_Custom_dmaen = function() {
    ...
    this.blitter_fillStruct = function() {
        ...
        t.BLTCON0(hpos, AMIGA_getCustom(BLTCON0));
        t.BLTCON1(hpos, AMIGA_getCustom(BLTCON1));

        t.BLTAFWM(hpos, AMIGA_getCustom(BLTAFWM));
        t.BLTALWM(hpos, AMIGA_getCustom(BLTALWM));

        t.BLTCPTH(hpos, AMIGA_getCustom(BLTCPTH));
        t.BLTCPTL(hpos, AMIGA_getCustom(BLTCPTL));
        t.BLTBPTH(hpos, AMIGA_getCustom(BLTBPTH));
        t.BLTBPTL(hpos, AMIGA_getCustom(BLTBPTL));
        t.BLTAPTH(hpos, AMIGA_getCustom(BLTAPTH));
        t.BLTAPTL(hpos, AMIGA_getCustom(BLTAPTL));
        t.BLTDPTH(hpos, AMIGA_getCustom(BLTDPTH));
        t.BLTDPTL(hpos, AMIGA_getCustom(BLTDPTL));

        t.BLTCMOD(hpos, AMIGA_getCustom(BLTCMOD));
        t.BLTBMOD(hpos, AMIGA_getCustom(BLTBMOD));
        t.BLTAMOD(hpos, AMIGA_getCustom(BLTAMOD));
        t.BLTDMOD(hpos, AMIGA_getCustom(BLTDMOD));

        t.BLTCDAT(hpos, AMIGA_getCustom(BLTCDAT));
        t.BLTBDAT(hpos, AMIGA_getCustom(BLTBDAT));
        t.BLTADAT(hpos, AMIGA_getCustom(BLTADAT));

        t.BLTSIZE(hpos, AMIGA_getCustom(BLTSIZE));
    }
	...
	/*function blitter_dump() {
		...
		SAEF_log("PC=%08X DMA=%d", SAER_CPU_getPC(), this.SAEF_Custom_dmaen());
	...
	function chipmem_agnus_wput2(addr, w) {
		...
		//if (!(log_blitter & 4))
		{
			...
			MACHINE.ram[addr] = w >> 8;
			MACHINE.ram[addr+1] = w & 0xff;
		}
	}*/
			...
			for (j = 0; j < blt_info.vblitsize; j++) {
				...
				for (i = 0; i < blt_info.hblitsize; i++) {
					...
					if (bltadatptr) {
						...
						blt_info.bltadat = bltadat = (MACHINE.ram[bltadatptr] << 8) | MACHINE.ram[bltadatptr + 1];
						bltadatptr += 2;
					} else
					...
					if (bltbdatptr) {
						...
						blt_info.bltbdat = bltbdat = (MACHINE.ram[bltbdatptr] << 8) | MACHINE.ram[bltbdatptr + 1];
						bltbdatptr += 2;
					...
					if (bltcdatptr) {
						...
						blt_info.bltcdat = (MACHINE.ram[bltcdatptr] << 8) | MACHINE.ram[bltcdatptr + 1];
						bltcdatptr += 2;
					}
					//if (dodst) chipmem_agnus_wput2(dstp, blt_info.bltddat);
					//if (dodst) SAER_Memory_chipPut16_indirect(dstp, blt_info.bltddat);
					if (dodst) {
						MACHINE.ram[dstp] = blt_info.bltddat >> 8;
						MACHINE.ram[dstp+1] = blt_info.bltddat & 0xff;
					}
	...
	function blitter_read() {
		if (bltcon0 & 0x200) {
			...
			blt_info.bltcdat = (MACHINE.ram[bltcpt] << 8) | MACHINE.ram[bltcpt + 1];
	...
	function blitter_write() {
		...
		if (bltcon0 & 0x200) {
			...
			MACHINE.ram[bltdpt] = blt_info.bltddat >> 8;
			MACHINE.ram[bltdpt+1] = blt_info.bltddat & 0xff;
		}
	...
	function blitter_doddma(hpos) {
		...
		MACHINE.ram[bltdpt] = d >> 8;
		MACHINE.ram[bltdpt+1] = d & 0xff;
		ptr.val = bltdpt;
	...
	function blitter_dodma(ch, hpos) {
		...
		switch (ch) {
			case 1:
				...
				blt_info.bltadat = (MACHINE.ram[bltapt] << 8) | MACHINE.ram[bltapt + 1];
			...
			case 2:
				...
				blt_info.bltbdat = (MACHINE.ram[bltbpt] << 8) | MACHINE.ram[bltbpt + 1];
	...
	this.decide_blitter = function(hpos) {
		...
		while (last_blitter_hpos < hpos) {
			...
			if (this.SAEF_Custom_dmaen() && !blit_final && (blitter_vcounter1 == blt_info.vblitsize || (blitter_vcounter1 == blt_info.vblitsize - 1 && blitter_hcounter1 == blt_info.hblitsize - 1 && blit_dmacount2 == 0))) {
	...
	function waitingblits() {
        return true;
		var waited = false;
		while (SAEV_Blitter_bltstate != SAEC_Blitter_bltstate_DONE && this.SAEF_Custom_dmaen()) {
			waited = true;
	...
	function do_blitter2(hpos = 0, copper = 0) {
		var cycles;
		...
		blit_firstline_cycles = blit_first_cycle = 0;
	...
	function do_blitter(hpos, copper) {
		...
		if (!this.SAEF_Custom_dmaen() || !blt_info.got_cycle)
			return;
	...
	function maybe_blit(hpos, hack) {
		...
		if (warned2 && this.SAEF_Custom_dmaen() && blt_info.got_cycle) {
			warned2--;
	...
	this.blitnasty = function() {
        blitter_cycle_exact = false;
		if (SAEV_Blitter_bltstate == SAEC_Blitter_bltstate_DONE) {
            debug("Blitter has nothing to do");
			return 0;
        }
		if (!this.SAEF_Custom_dmaen()) {
            debug("Blitter DMA is not enabled");
			return 0;
        }
		if (blitter_cycle_exact) {
...
//		maybe_blit(hpos, 0);
	...
	function blitdofast_0(pta, ptb, ptc, ptd, b) {
		...
		for (j = 0; j < b.vblitsize; j++) {
			for (i = 0; i < b.hblitsize; i++) {
				var bltadat, srca;
				if (dstp) {
					MACHINE.ram[dstp] = dstd >> 8;
					MACHINE.ram[dstp + 1] = dstd & 0xff;
				}
				dstd = 0 & 0xffff;
				totald |= dstd;
	...
	function blitdofast_a(pta, ptb, ptc, ptd, b) {
		...
		for (j = 0; j < b.vblitsize; j++) {
			for (i = 0; i < b.hblitsize; i++) {
				var bltadat, srca;
				if (ptc) { srcc = (MACHINE.ram[ptc] << 8) | MACHINE.ram[ptc + 1]; ptc += 2; }
				if (pta) { bltadat = blt_info.bltadat = (MACHINE.ram[pta] << 8) | MACHINE.ram[pta + 1]; pta += 2; } else { bltadat = blt_info.bltadat; }
				bltadat &= blit_masktable[i];
	...
	function blitdofast_2a(pta, ptb, ptc, ptd, b) {
		...
		for (j = 0; j < b.vblitsize; j++) {
			for (i = 0; i < b.hblitsize; i++) {
				...
				if (ptc) { srcc = (MACHINE.ram[ptc] << 8) | MACHINE.ram[ptc + 1]; ptc += 2; }
				if (ptb) {
					var bltbdat = blt_info.bltbdat = (MACHINE.ram[ptb] << 8) | MACHINE.ram[ptb + 1]; ptb += 2;
